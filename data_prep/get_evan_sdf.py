...
def export_data(split_ids: Dict, save_data=True, start=0, end=0,
    ...
    hdf5_file.create_dataset(
        'value', [num_shapes, pt_sample_res, pt_sample_res, pt_sample_res],
        dtype=np.float32)
    hdf5_file.create_dataset(
        'occ', [num_shapes, pt_sample_res, pt_sample_res, pt_sample_res],
        dtype=np.float32)
    hdf5_file.create_dataset(
    ...
    for i, anno_id in enumerate(tqdm(all_valid_anno_ids)):
        ...
        hdf5_file['value'][i] = out['values']
        hdf5_file['occ'][i] = out['occ']
        hdf5_file['param'][i] = out['param']
...
def make_data_for_one(anno_id,
                      ...
                      ori_ids_to_new_ids,
                      vis=False):
    verts = entire_mesh.vertices

    fg_voxels, _ = get_info_from_voxels(
        anno_id, pt_sample_res, entire_mesh)

    sdf_grid = ops.bin2sdf(input=fg_voxels)
    values = np.swapaxes(sdf_grid, 0, 2)
    occ = np.where(values <= 0, 1, 0)
    
    # np.save(f'{anno_id}.npy', values)
...
if __name__ == "__main__":
    ...
    # print([train_ids[x] for x in [2, 5, 6, 9, 4]])
    ...
    export_data([train_ids[x] for x in [2, 5, 6, 9, 4]], save_data=True, start=0, end=5)
    exit(0)
