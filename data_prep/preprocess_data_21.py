...
def build_dense_graph(union_root: AnyNode, name_to_obbs: Dict, obbs,
    ...
    if cat_name == 'Chair':
        ...
        # # for 24 shapes
        # connectivity = [(0, 1), (1, 2), (1, 3), (2, 3), (3, 4), (3, 5)]
    if cat_name == 'Lamp':
...
def merge_partnet_after_merging(anno_id, info=False):
    ...
    if merged_parts_info == {}:
        valid = False
        return None, None, None, None, None, None, None, None, anno_id, valid
...
def export_data(split_ids: Dict, save_data=True, start=0, end=0,
    ...
    #         hdf5_file['relations'][idx] = all_relations[idx]
    #         hdf5_file['verts'][idx] = all_norm_meshes[idx].vertices.flatten()
    #         hdf5_file['faces'][idx] = all_norm_meshes[idx].faces.flatten()
    #         pbar.update(1)
    #     all_exited = True
    #     for p in workers:
    #         if p.exitcode is None:
...
def make_data_for_one(anno_id,
    ...
    fg_voxels = fill_internal_voxels(torch.from_numpy(fg_voxels)).numpy()
    ...
    pv_indices = list(range(len(points)))
    # np.random.seed(319)
    ...
    part_num_indices, all_indices = convert_fg_part_indices_to_flat_list(
        fg_part_indices)
    points = torch.from_numpy(points).to('cpu', dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    # exit(0)
    # print(train_and_val[:36])
    # exit(0)

    unique_name_to_new_id, all_entire_meshes, all_part_meshes,\
        all_ori_ids_to_new_ids, all_obbs, all_name_to_obbs =\
                export_data(train_and_val, save_data=False, start=0, end=36)

    with open('data/Chair_train_new_ids_to_objs_20_0_36.json', 'r') as f:
        all_obs = json.load(f)
