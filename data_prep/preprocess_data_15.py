...
def merge_partnet_after_merging(anno_id, info=False):
    ...
    with open('data_prep/further_merge_info_15.json', 'r') as f:
...
def export_data(split_ids: Dict, save_data=True, start=0, end=0,
    ...
    with open(
        f'data/{cat_name}_part_name_to_new_id_15_{start}_{end}.json',
        'w') as f:
    ...
    with open(
        f'data/{cat_name}_train_new_ids_to_objs_15_{start}_{end}.json',
        'w') as f:
    ...
    UniqueDotExporter(union_root_part,
                      ...
                      nodeattrfunc=lambda node: "shape=box",).to_picture(
                          f"data_prep/tmp/tree_union_class_15_{start}_{end}.png")
    ...
    np.save(f'data/{cat_name}_union_node_names_15_{start}_{end}.npy',
            union_node_names)
    ...
    UniqueDotExporter(recon_root,
                      ...
                      nodeattrfunc=lambda node: "shape=box",
                          ...
                          f"data_prep/tmp/recon_tree_union_15_{start}_{end}.png")
    ...
    fn = f'data/{cat_name}_train_{pt_sample_res}_15_{start}_{end}.hdf5'
    ...
    # # pyg = PartPtsDataset(f'data/{cat_name}_train_{pt_sample_res}_15_{start}_{end}',
    # #                      all_pts_data)
    # # pyg.process()

    # pyg = PartPtsDataset(f'data/{cat_name}_train_{pt_sample_res}_15_{start}_{end}_whole',
    #                      all_pts_whole_data)
    # pyg.process()
...
if __name__ == "__main__":
    # # pass one to create the preprocess_15 dataset
    # export_data(train_ids, save_data=False, start=0, end=2000)
    # exit(0)
    
    # pass two to create the four_parts dataset
    # good_indices = np.load('data/chair_am_four_parts_15_0_2000.npy')
    data_pt = 'data/Chair_train_new_ids_to_objs_15_0_2000.json'
    ...
    export_data(ids, save_data=True,
                start=0, end=len(ids))
                # start=0, end=10)
    exit(0)
