...
def merge_partnet_after_merging(anno_id, info=False):
    ...
    with open('data_prep/further_merge_info_14.json', 'r') as f:
...
def export_data(split_ids: Dict, save_data=True, start=0, end=0,
    ...
    with open(
        f'data/{cat_name}_part_name_to_new_id_14_{start}_{end}.json',
        'w') as f:
    ...
    with open(
        f'data/{cat_name}_train_new_ids_to_objs_14_{start}_{end}.json',
        'w') as f:
    ...
    UniqueDotExporter(union_root_part,
                      ...
                      nodeattrfunc=lambda node: "shape=box",).to_picture(
                          f"data_prep/tmp/tree_union_class_14_{start}_{end}.png")
    ...
    np.save(f'data/{cat_name}_union_node_names_14_{start}_{end}.npy',
            union_node_names)
    ...
    UniqueDotExporter(recon_root,
                      ...
                      nodeattrfunc=lambda node: "shape=box",
                          ...
                          f"data_prep/tmp/recon_tree_union_14_{start}_{end}.png")
    ...
    fn = f'data/{cat_name}_train_{pt_sample_res}_14_{start}_{end}.hdf5'
...
if __name__ == "__main__":
    # # pass one to create the preprocess_14 dataset
    # export_data(train_ids, save_data=False, start=0, end=10)
    # exit(0)
    
    # pass two to create the four_parts dataset
    good_indices = np.load('data/chair_am_four_parts_14_0_2000.npy')
