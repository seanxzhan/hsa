...
def export_data(split_ids: Dict, save_data=True, start=0, end=0,
    ...
    with open(
        f'data/{cat_name}_part_name_to_new_id_13_{start}_{end}.json',
        'w') as f:
    ...
    with open(
        f'data/{cat_name}_train_new_ids_to_objs_13_{start}_{end}.json',
        'w') as f:
    ...
    UniqueDotExporter(union_root_part,
                      ...
                      nodeattrfunc=lambda node: "shape=box",).to_picture(
                          f"data_prep/tmp/tree_union_class_13_{start}_{end}.png")
    ...
    np.save(f'data/{cat_name}_union_node_names_13_{start}_{end}.npy',
            union_node_names)
    ...
    UniqueDotExporter(recon_root,
                      ...
                      nodeattrfunc=lambda node: "shape=box",
                          ...
                          f"data_prep/tmp/recon_tree_union_13_{start}_{end}.png")
    ...
    fn = f'data/{cat_name}_train_{pt_sample_res}_13_{start}_{end}.hdf5'
    ...
    # # pyg = PartPtsDataset(f'data/{cat_name}_train_{pt_sample_res}_13_{start}_{end}',
    # #                      all_pts_data)
    # # pyg.process()

    # pyg = PartPtsDataset(f'data/{cat_name}_train_{pt_sample_res}_13_{start}_{end}_whole',
    #                      all_pts_whole_data)
    # pyg.process()
...
if __name__ == "__main__":
    # # pass one to create the preprocess_13 dataset
