...
from utils import tree, visualize, misc
...
partnet_dir = 'toy_dataset/PartNet'
stats_dir = 'toy_dataset/partnet_dataset/stats'
...
def merge_partnet_after_merging(anno_id, info=False):
    ...
    # with open(f'toy_dataset/tmp/{anno_id}_parts_info_before.json', 'w') as f:
    ...
    if info:
        ...
        with open(f'toy_dataset/tmp/{anno_id}_parts_info.json', 'w') as f:
            json.dump(parts_info, f)

    with open('toy_dataset/further_merge_info.json', 'r') as f:
    ...
    if info:
        with open(f'toy_dataset/tmp/{anno_id}_merged_parts_info.json', 'w') as f:
    ...
    if info:
        with open(f'toy_dataset/tmp/{anno_id}_part_map.json', 'w') as f:
...
def export_data(split_ids: Dict, save_data=True, start=0, end=0,
    ...
    with open(
        f'toy_dataset/{cat_name}_part_name_to_new_id_ori_merged_{start}_{end}.json',
        'w') as f:
    ...
    with open(
        f'toy_dataset/{cat_name}_train_new_ids_to_objs_ori_merged_{start}_{end}.json',
        'w') as f:
    ...
    UniqueDotExporter(union_root_part,
                      ...
                      nodeattrfunc=lambda node: "shape=box",).to_picture(
                          f"toy_dataset/tmp/tree_union_class_ori_merged_{start}_{end}.png")
    ...
    np.save(f'toy_dataset/{cat_name}_union_node_names_ori_merged_{start}_{end}.npy',
            union_node_names)
    ...
    UniqueDotExporter(recon_root,
                      ...
                      nodeattrfunc=lambda node: "shape=box",
                          ...
                          f"toy_dataset/tmp/recon_tree_union_ori_merged_{start}_{end}.png")
    ...
    # exit(0)
    fn = f'toy_dataset/{cat_name}_train_ori_merged_{start}_{end}.hdf5'
    ...
    hdf5_file.create_dataset(
        ...
        dtype=np.float32)
    hdf5_file.create_dataset(
        'obbs', [num_shapes, num_parts, OBB_REP_SIZE],
        dtype=np.float32)
    ...
    for i, anno_id in enumerate(tqdm(all_valid_anno_ids)):
        out = make_data_for_one(unique_name_to_new_id,
                                all_name_to_obbs[i])
        hdf5_file['obbs'][i] = out['obbs']
...
def make_data_for_one(unique_name_to_new_id: Dict,
                      name_to_obbs: Dict,
                      vis=False):
    # use unique_name_to_new_id and name_to_obb_indices to arrange
    # the bounding boxes for each shape
    part_obbs = []
    ...
    for un in unique_names:
        if not un in model_part_names:
            part_obbs.append(torch.zeros((1, OBB_REP_SIZE)).to(torch.float32))
            continue
        # print(name_to_obbs[un])
        part_obbs.append(condense_obbs([name_to_obbs[un]]))

    compact_obbs = torch.concat(part_obbs, dim=0).numpy()

    return {
        # 1, num_part_classes, max_num_instances_per_part, OBB_REP_SIZE
        'obbs': compact_obbs,
    }
