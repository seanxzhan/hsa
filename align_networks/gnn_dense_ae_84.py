...
class GraphDecoder(torch.nn.Module):
    def __init__(self, input_dims, num_nodes,
        ...
        lift_net = (
            torch.nn.Linear(input_dims, 2 * input_dims, bias=False),
            torch.nn.ReLU())
        for _ in range(node_feature_hidden-1):
            lift_net = lift_net + (
                torch.nn.Linear(2 * input_dims, 2 * input_dims, bias=False),
                torch.nn.ReLU())
        lift_net = lift_net + (torch.nn.Linear(2 * input_dims, 2 * input_dims, bias=False),)
        self.lift_net = torch.nn.Sequential(*lift_net)
    ...
    def forward(self, z):
        lifted_input = self.lift_net(z)
        node_feats = self.node_net(lifted_input[:, :self.input_dims]).view(-1, self.num_nodes, self.node_feature_dims)
        edge_feats = self.edge_net(lifted_input[:, self.input_dims:]).view(-1, self.num_nodes, self.num_nodes, self.node_feature_dims)
