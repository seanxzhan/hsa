...
class SDFDecoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # refine_out_dims = 4
        self.refine_net_sdf = SmallMLPs(input_dims,
                                        self.vol_cell_dim,
                                        internal_dims,
                                        hidden,
                                        1,
                                        multires)
        self.refine_net_def = SmallMLPs(input_dims,
                                        self.vol_cell_dim,
                                        internal_dims,
                                        hidden,
                                        3,
    ...
    def get_sdf_deform(self, points, features=None):
        if features is not None:
            feat_vol = self.feature_volume(features).view(
                -1, self.vol_cell_dim, points.shape[0]).transpose(1, 2)
            expanded_points = points.unsqueeze(0).expand(feat_vol.shape[0], -1, -1)
            sdf = self.refine_net_sdf.forward(expanded_points, feat_vol)
            deform = self.refine_net_def.forward(expanded_points, feat_vol)
            return sdf, deform
        else:
            return self.refine_net_sdf.forward(points,
                torch.zeros((points.shape[0], self.vol_cell_dim)).to('cuda')), \
                self.refine_net_sdf.forward(points,
                torch.zeros((points.shape[0], self.vol_cell_dim)).to('cuda'))
    ...
    def pre_train_sphere(self, iter):
        ...
        for i in tqdm(range(iter)):
            ...
            sdf, _ = self.get_sdf_deform(p)
            loss = loss_fn(torch.tanh(sdf[...,0]), ref_value)
