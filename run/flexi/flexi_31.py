...
def run_flexi(gt_mesh_idx, s, sdf_deform, pred_occ, test=False):
    ...
    # target = render.render_mesh_paper(my_load_mesh(gt_mesh_idx), mv, mvp, train_res)
...
if args.train:
    for it in range(iterations):
        ...
        for b in range(nb):
            ...
            loss_occ = loss_f(pred_occ,
                              torch.from_numpy(occ[b*bs:(b+1)*bs]).to(device))
            ...
            for s in range(bs):
                one_mesh_loss, grid_verts, sdf, vertices, faces =\
                    run_flexi(b*bs+s, s, sdf_deform, pred_occ)
            ...
            itr_occ_loss += loss_occ.detach().cpu().numpy()
            itr_mesh_loss += mesh_loss.detach().cpu().numpy()
