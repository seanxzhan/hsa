...
from occ_networks.flexi_decoder_30 import SDFDecoder as FlexiDecoder
...
def run_flexi(gt_mesh_idx, s, sdf_deform, gt_meshes, test=False):
    ...
    sdf, deform = sdf_deform[s, :, :1], sdf_deform[s, :, 1:]
...
if args.train:
    for it in range(iterations):
        ...
        for b in range(nb):
            ...
            sdf_deform = model.get_sdf_deform(x_nx3)
            
            all_loss = 0
            for s in range(10):
                one_mesh_loss, grid_verts, sdf, vertices, faces =\
                    run_flexi(b*bs+s, s, sdf_deform, gt_meshes)
