...
if not OVERFIT:
    ...
    flexi_ckpt_dir = os.path.join(logs_path, 'flexi-ckpt')
...
class PPNetVol(torch.nn.Module):
    def __init__(self, hidden=0, internal_dims=64):
        super().__init__()

        refine_out_dims = 4
        self.refine_net = SmallMLPs(3,
                                    16,
                                    internal_dims,
                                    hidden,
                                    refine_out_dims,
        ...
        self.feature_volume = VoxDecoder(feature_size=128)

    def forward(self, points, features, final_layer=True):
        feat_vol = self.feature_volume(features).view(-1, 16, 32**3).transpose(1, 2)
        x = self.refine_net.forward(
            points,
            feat_vol)
        if final_layer:
            x = torch.nn.Tanh()(x)
...
sdf_deform_model = PPNetVol(hidden=5, internal_dims=128).to(device)
sdf_deform_model_params = [p for _, p in sdf_deform_model.named_parameters()]
...
embeddings2 = torch.nn.Embedding(num_shapes,
                                 128).to(device)
if args.train:
    torch.nn.init.normal_(
        embeddings2.weight.data,
        0.0,
        1 / math.sqrt(num_parts*each_part_feat),
    )

# it = args.it
checkpoint = torch.load(os.path.join(ckpt_dir, f'model_{3000}.pt'))
...
def load_batch(batch_idx, batch_size):
    ...
    return all_fg_part_indices[start:end],\
        ...
        embeddings2(torch.arange(start, end).to(device)),\
...
# optimizer = torch.optim.Adam([{"params": deform_params, "lr": lr}])
optimizer = torch.optim.Adam([{"params": embeddings2.parameters(), "lr": lr},
                              {"params": sdf_deform_model_params, "lr": lr}])
...
def train_one_itr(it, b,
    ...
    # delta_sdf = sdf_deform_model(batch_embed)

    flexi_out = sdf_deform_model.forward(batch_points, batch_embed2)

    pred_sdf = sdf + flexi_out[:, :, :1]
    # pred_sdf = flexi_out[:, :, :1]
    # pred_sdf = pred_values2
    deform = flexi_out[:, :, 1:]
    ...
    for ele in range(batch_size):
        ...
        # delta_sdf = sdf_deform_model.forward(batch_embed_sdf_deform,
        #                                      final_layer=True).squeeze(-1)

        grid_verts = x_nx3 + (2-1e-8) / (fc_voxel_grid_res * 2) * torch.tanh(deform[ele])
        vertices, faces, L_dev = fc(
            grid_verts, torch.squeeze(pred_sdf[ele]),
    ...
    if b == n_batches - 1:
        writer.add_scalar('mask loss', mask_loss, it)
        writer.add_scalar('depth loss', depth_loss, it)
...
if args.train:
    ...
    for it in range(iterations):
        ...
        if (it) % save == 0 or it == (iterations - 1):
            torch.save({
                'epoch': it,
                'model_state_dict': sdf_deform_model.state_dict(),
                'embeddings_state_dict': embeddings2.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'avg_batch_loss': avg_batch_loss,
                }, os.path.join(flexi_ckpt_dir, f'model_{it}.pt'))
...
if args.post_process_fc:
    ...
    for it in range(iterations):
        ...
        total_loss += dmtet_utils.loss_f(iterations, vertices, faces, pcd, it)
