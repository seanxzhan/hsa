...
def load_batch(batch_idx, batch_size):
    ...
    return all_fg_part_indices[start:end],\
        ...
        torch.from_numpy(part_nodes[chair_arm_indices[start:end]]).to(device, torch.long),\
        embeddings(torch.arange(start, end).to(device)),\
        torch.from_numpy(empty_parts[chair_arm_indices[start:end]]).to(device, torch.long)
