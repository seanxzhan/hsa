...
from data_prep import preprocess_data_13
...
train_new_ids_to_objs_path = f'data/{cat_name}_train_new_ids_to_objs_13_{ds_start}_{ds_end}.json'
...
train_data_path = f'data/{cat_name}_train_{pt_sample_res}_13_{ds_start}_{ds_end}.hdf5'
...
for i in range(num_shapes):
    indices = preprocess_data_13.convert_flat_list_to_fg_part_indices(
...
def load_minibatch(batch_idx, batch_size, minib_idx, minib_size):
    s = batch_idx*batch_size
    e = s + batch_size
    ms = minib_idx*minib_size
    me = ms + minib_size
    return all_fg_part_indices[s:e],\
        torch.from_numpy(normalized_points[s:e, ms:me]).to(device, torch.float32),\
        torch.from_numpy(values[s:e, ms:me]).to(device, torch.float32),\
        embeddings(torch.arange(s, e).to(device)),\
        torch.from_numpy(node_features[s:e, :, :3]).to(device, torch.float32),\
        torch.from_numpy(adj[s:e]).to(device, torch.long),\
        torch.from_numpy(part_nodes[s:e]).to(device, torch.long),\
        torch.from_numpy(xforms[s:e, :, :3, 3]).to(device, torch.float32),\
        torch.from_numpy(relations[s:e, :, :3, 3]).to(device, torch.float32)
...
def train_one_minib(it, b, minib,
    ...
    if b == n_batches - 1 and minib == n_minib - 1:
    ...
    for it in range(iterations):
        ...
        for b in range(n_batches):
            ...
            for minib in range(n_minib):
                ...
                writer.add_scalar('iteration loss', minib_loss, it)
...
if args.test:
    ...
    unique_part_names, name_to_ori_ids_and_objs,\
        ...
        preprocess_data_13.merge_partnet_after_merging(anno_id)

    with open(f'data/{cat_name}_part_name_to_new_id_13_{ds_start}_{ds_end}.json', 'r') as f:
...
if args.asb:
    ...
    node_names = np.load(f'data/{cat_name}_union_node_names_13_{ds_start}_{ds_end}.npy')
