...
if args.test:
    ...
    pred_mesh.export(os.path.join(results_dir, f'mesh_pred-{parts_str}.obj'))
    ...
    if not args.mask:
        ...
        aligned_gt_mesh.export(os.path.join(results_dir, f'mesh_gt-{parts_str}.obj'))
        aligned_gt_mesh.visual.vertex_colors = [31, 119, 180]
        visualize.save_mesh_vis(aligned_gt_mesh, mesh_gt_path,
                                mag=mag, white_bg=False)
...
if args.asb:
    ...
    # model_indices = [0, 0, 0, 0]
    # part_indices = [0, 1, 2, 3]

    # model_indices = [11] * 4
    # part_indices = [0, 1, 2, 3]

    # model_indices = [27, 9, 39, 37]
    # part_indices = [2, 3, 1, 0]

    # model_indices = [86, 47, 39, 37]
    # part_indices = [2, 3, 1, 0]

    # model_indices = [86, 43, 39, 37]
    # part_indices = [2, 3, 1, 0]

    # model_indices = [86, 43, 19, 37]
    # part_indices = [2, 3, 1, 0]

    # model_indices = [86, 43, 47, 37]
    # part_indices = [2, 3, 1, 0]

    # model_indices = [86, 43, 12, 37]
    # part_indices = [2, 3, 1, 0]

    # model_indices = [86, 43, 37, 39]
    # part_indices = [2, 3, 1, 0]

    # model_indices = [86, 43, 37, 41]
    # part_indices = [2, 3, 1, 0]

    # model_indices = [37, 86, 19, 41]
    # part_indices = [2, 3, 1, 0]
    ...
    for i, idx in enumerate(model_indices):
        ...
        shape_ext = torch.from_numpy(extents[idx, part_idx])
    ...
    # learned_xforms = gt_xforms.clone()
    # learned_xforms[0, 0] -= torch.tensor([0, 0, 0.1])
    # learned_xforms[0, 1] -= torch.tensor([0, -0.2, 0.1])
    # learned_xforms[0, 2] -= torch.tensor([0, -0.3, 0.1])
    # learned_xforms[0, 3] -= torch.tensor([0, 0, 0])

    # assembly model_indices = [37, 86, 43, 41]
    # learned_xforms = gt_xforms.clone()
    # learned_xforms[0, 0] -= torch.tensor([0, 0, 0.1])
    # learned_xforms[0, 1] -= torch.tensor([0, -0.3, 0.1])
    # learned_xforms[0, 2] -= torch.tensor([0, -0.3, 0.1])
    # learned_xforms[0, 3] -= torch.tensor([0, 0, 0])

    # assembly model_indices = [39, 86, 43, 41]
    learned_xforms = gt_xforms.clone()
    learned_xforms[0, 0] -= torch.tensor([0, 0.05, 0.1])
    learned_xforms[0, 1] -= torch.tensor([0, -0.2, 0.1])
    learned_xforms[0, 2] -= torch.tensor([0, -0.05, 0.05])
    learned_xforms[0, 3] -= torch.tensor([0, 0, 0])
    ...
    learned_xforms = learned_xforms.to(device)
    ...
    visualize.save_obbs_vis(prex_obbs_of_interest,
                            prex_obbs_path, mag=0.8, white_bg=False)
