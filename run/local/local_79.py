...
def train_one_itr(it, b,
    ...
    points_mask = 1 - batch_parts.unsqueeze(-1).unsqueeze(-1) + batch_parts.unsqueeze(-1).unsqueeze(-1) * points_mask

    occ_mask = torch.zeros((batch_size, 1, num_parts)).to(device, torch.float32)
    occ_mask[:, :, rand_indices] = 1
    occ_mask = 1 - batch_parts.unsqueeze(1) + batch_parts.unsqueeze(1) * occ_mask
