...
from occ_networks.basic_decoder_nasa_7 import SDFDecoder
...
fake_points = np.zeros((num_shapes, 45, num_points, 3), dtype=np.float32)
transformed_points = np.concatenate((transformed_points[:], fake_points), axis=1)
...
embeddings = torch.nn.Embedding(num_shapes,
                                48*each_part_feat).to(device)
if args.train:
    torch.nn.init.normal_(
        embeddings.weight.data,
        0.0,
        1 / math.sqrt(48*each_part_feat),
    )
...
empty_parts_mask = torch.zeros((batch_size, num_points, 48),
                               requires_grad=False).to(device, torch.float32)
empty_parts_mask[:, :, 3:] = 1
...
def train_one_itr(it, all_fg_part_indices, 
    ...
    parts_mask = torch.ones((batch_embed.shape[0], 48)).to(device, torch.float32)
    ...
    occs1 = model(transformed_points,
                  modified_embed).masked_fill(empty_parts_mask==1,
    ...
    occs2 = model(transformed_points,
                  batch_embed).masked_fill(empty_parts_mask==1,
                                           torch.tensor(float('-inf')))
...
if args.test:
    ...
    fake_points = torch.zeros(1, 45, num_points, 3).to(device, torch.float32)
    transformed_points = torch.concat((transformed_points[:], fake_points), dim=1)

    empty_parts_mask = torch.zeros((1, num_points, 48),
                                   requires_grad=False).to(device, torch.float32)
    empty_parts_mask[:, :, 3:] = 1
