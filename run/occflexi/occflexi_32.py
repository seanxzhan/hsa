...
from occ_networks.occflexi_gnn_18 import GNN
...
parser.add_argument('--vis_spa_inv', action="store_true")
parser.add_argument('--vis_wavelet_gen', action="store_true")
...
all_verts = train_data['verts']
all_faces = train_data['faces']
...
def my_load_mesh_vf(model_idx):
    v = np.array(all_verts[model_idx]).reshape(-1, 3)
    f = np.array(all_faces[model_idx]).reshape(-1, 3)
...
if args.train:
    ...
    gt_meshes = [my_load_mesh_vf(s) for s in tqdm(range(0, num_shapes))]
    gt_mesh = my_load_mesh_vf(num_shapes + anchor_idx)
...
if REFINE:
    gnn_model = GNN(num_parts=num_parts, feature_dims=each_part_feat).to(device)
    gnn_model_params = [p for _, p in gnn_model.named_parameters()]
...
def load_batch_no_embed(batch_idx, batch_size, start=None, end=None):
    if start is None:
        start = batch_idx*batch_size
    if end is None:
        end = start + batch_size
...
def run_occ(batch_size, masked_indices, batch_points,
    ...
    if REFINE:
        ...
        learned_xforms = gnn_model.learn_geom_xform(batch_node_feat,
                                                    batch_adj,
                                                    batch_mask,
                                                    batch_vec)
    ...
    if not REFINE:
        return learned_xforms, None,\
            pred_values1, pred_values2, entire_verts_values, comp_sdf, part_sdf
    else:
...
def inference_network(masked_indices, query_points,
                      ...
                      fixed_indices=None):
    with torch.no_grad():
        learned_xforms, _, pred_values1, _, _, comp_sdf, _ =\
...
# ------------ training ------------
if args.train:
    starting_it = 0 if not args.restart else args.it+1
    ...
    # for it in range(starting_it, starting_it+100):
        ...
        for b in range(num_batches):
            ...
            pred_values1, pred_values2, pred_verts_occ, comp_sdf, part_sdf =\
            ...
            if REFINE:
                ...
                # learned_mesh_relations = pairwise_xforms[:, :, 1] - pairwise_xforms[:, :, 0]
                total_loss += 10 * loss_xform + 10 * loss_relations + 10 * loss_mesh_xform + loss_bbox_geom
        ...
        if REFINE:
            ...
            writer.add_scalar('bbox_geom', loss_bbox_geom, it)
...
# ------------ one shape bbox info ------------
def infer_bbox_info(args, query_points, flexi_verts, fc_res, x_nx3, cube_fx8,
                    custom_embed=None):
    ...
    return learned_xforms[0].cpu().numpy(), learned_geom
...
def recon_one_shape(anno_id, results_dir, args,
    ...
    for p in range(num_parts):
        ...
        if occ_vertices[0].cpu().shape[0] == 0:
            learned_geom.append(np.array([0.0, 0.0, 0.0]))
            continue
        part_mesh = trimesh.Trimesh(
            part_flexi_vertices.cpu().numpy(),
    ...
    # flexi_mesh = trimesh.Trimesh(flexi_vertices.numpy(), flexi_faces.numpy())
    # flexi_bbox_ext = flexi_mesh.bounding_box.primitive.extents
    # print(flexi_bbox_ext)
    ...
    if recon_gt and not mask_gt:
        mesh_gt = ops.export_mesh_norm(torch.from_numpy(gt_mesh.vertices),
                                       torch.from_numpy(gt_mesh.faces),
                                       os.path.join(results_dir, 'mesh_gt.obj'))
        # mesh_gt_bbox_ext = mesh_gt.bounding_box.primitive.extents
        # print(mesh_gt_bbox_ext)
        # exit(0)
        mesh_gt.visual.vertex_colors = gt_color
        visualize.save_mesh_vis(mesh_gt, mesh_gt_path,
                                mag=mag, white_bg=white_bg)
...
if args.asb_scaling:
    ...
    _, _, _, _, anchor_node_feat, _, anchor_part_nodes, _, _ =\
        load_batch(0, 0, anchor_model_idx, anchor_model_idx+1)
    anchor_bbox_geom = torch.einsum('ijk, ikm -> ijm',
                                    anchor_part_nodes.to(torch.float32),
                                    anchor_node_feat)[0].cpu().numpy()
    ...
    for i in range(num_parts):
        ...
        gt_xform = np.eye(4)
        gt_xform[:3, 3] = -gt_xforms[i]
        learned_ext = learned_geom[i] * scales[i]
        ext_xform = (learned_ext, learned_xform)
        prex_ext_xform = (ext, gt_xform)
        obbs_of_interest[i] = [ext_xform]
        prex_obbs_of_interest[i] = [prex_ext_xform]
...
if args.inv:
    ...
    if not os.path.exists(occ_embedding_fp):
        ...
        for i in pbar:
            ...
            batch_node_feat = torch.einsum(
                'ijk, ikm -> ijm',
                batch_part_nodes.to(torch.float32).swapaxes(1, 2), # 1 7 4 
                batch_occ_embed.detach().view(1, num_parts, each_box_feat)) # 1 4 32
    ...
    copyfile(os.path.join(results_dir, 'mesh_gt.obj'),
             os.path.join('/sota/spaghetti/assets/mesh', f'{anno_id}.obj'))
    exit(0)
...
if args.asb_scaling_inv:
    ...
    # inv_mode = f'{mode}_just_occ'
    # inv_mode = f'{mode}_one_img'
    inv_mode = f'mesh'
    ...
    # ------------ look for embedding ------------ 
    occ_embeddings = torch.nn.Embedding(len(anno_ids), num_parts*each_part_feat).to(device)
    for i, anno_id in enumerate(anno_ids):
        path_to_embed = os.path.join(results_dir, f'inv_{inv_mode}', anno_id, 'occ_embedding.pth')
        one_embed = torch.load(path_to_embed)
        occ_embeddings.weight.data[i] = one_embed.weight.data
    ...
    for i, idx in enumerate(model_indices):
        args.test_idx = idx
        xfm, ext = infer_bbox_info(args, query_points, flexi_verts, fc_res, x_nx3, cube_fx8,
                                   occ_embeddings(torch.arange(i,i+1).to(device)))
        source_xforms.append(xfm)
        source_exts.append(ext)
    ...
    #                                 anchor_part_nodes.to(torch.float32),
    #                                 anchor_node_feat)[0].cpu().numpy()
    anchor_bbox_geom = np.concatenate(source_exts[idx_anchor]).reshape(num_parts, 3)
    # anchor_bbox_geom = torch.from_numpy(anchor_bbox_geom)

    # ------------ anno ids and model indices mapping ------------
    # asb_str = '-'.join([str(x) for x in anno_ids+part_indices])
    anno_str = '-'.join([str(x) for x in anno_ids])
    parts_str = '-'.join([str(x) for x in part_indices])
    results_dir = os.path.join(results_dir, f'asb_scaling_inv_{inv_mode}', anno_str, parts_str)
    misc.check_dir(results_dir)
    print("results dir: ", results_dir)
    with open(os.path.join(results_dir, 'ids.txt'), 'w') as f:
        anno_ids_str = '-'.join([str(x) for x in anno_ids])
    ...
    for i, idx in enumerate(model_indices):
        ...
        for uni in union_node_indices:
            build_adj_matrix(uni, asb_adj, shape_adj)
        asb_occ_embed[:, part_idx*each_part_feat:(part_idx+1)*each_part_feat] =\
            occ_embeddings(torch.tensor(i).to(device)).cpu()[part_idx*each_part_feat:(part_idx+1)*each_part_feat]
        gt_xforms[:, part_idx] = shape_xform
        gt_exts[part_idx] = shape_ext
    node_names = np.load(
        f'data/{cat_name}_union_node_names_{ds_id}_{ds_start}_{ds_end}.npy')
...
if args.inv_mesh:
    ...
    mesh_name = f'{anno_id}_out'
    mesh_dir = '/sota/SDFusion/results'
    # model_idx = args.test_idx
    # anno_id = model_idx_to_anno_id[model_idx]
    # model_id = misc.anno_id_to_model_id(partnet_index_path)[anno_id]
    # print(f"anno id: {anno_id}, model id: {model_id}")
    ...
    results_dir = os.path.join(results_dir, f'inv_{inv_mode}', anno_id)
    misc.check_dir(results_dir)
    print("results dir: ", results_dir)
    ...
    fg_voxels, points, values = ops.get_vox_from_one_mesh(mesh_name, mesh_dir, 64)
    ...
    points = torch.from_numpy(points[None, :]).to(device, torch.float32)
    batch_points = kaolin.ops.pointcloud.center_points(points, normalize=True)
    batch_values = torch.from_numpy(values[None, :]).to(device, torch.float32)
    batch_adj = torch.from_numpy(adj[2:2+1]).to(device, torch.float32)
    batch_part_nodes = torch.from_numpy(part_nodes[2:2+1]).to(device, torch.float32)
    ...
    if not os.path.exists(occ_embedding_fp):
        ...
        for i in pbar:
            ...
            pred_values2, _ = torch.max(occ_model.forward(
                transformed_points, batch_occ_embed), dim=-1, keepdim=True)
            total_loss = loss_f(pred_values2, batch_values)
...
if args.vis_spa_inv:
    from utils import visualize
    results_dir = os.path.join(results_dir, 'spaghetti', 'inversion')
    misc.check_dir(results_dir)

    inverted_mesh_dir = '/sota/spaghetti/assets/checkpoints/spaghetti_chairs_large/inversion/occ'
    mesh_names = list(filter(lambda x: os.path.splitext(x)[1] == '.obj',
                             os.listdir(inverted_mesh_dir)))
    print(mesh_names)
    for mn in mesh_names:
        anno_id = os.path.splitext(mn)[0]
        path = os.path.join(inverted_mesh_dir, mn)
        mesh = trimesh.load_mesh(path)
        mesh_norm_dir = os.path.join(results_dir, anno_id); misc.check_dir(mesh_norm_dir)
        print(mesh_norm_dir)
        mesh_norm_path = os.path.join(mesh_norm_dir, mn)
        mesh_norm = ops.export_mesh_norm(torch.from_numpy(mesh.vertices),
                                         torch.from_numpy(mesh.faces),
                                         mesh_norm_path)
        visualize.save_mesh_vis(mesh_norm,
                                os.path.join(results_dir, anno_id, f'{anno_id}_spa_inv.png'),
                                mag=0.8, white_bg=True)
        copyfile(os.path.join('results/occflexi/occflexi_25/inv_test_just_occ',
                              anno_id, f'{anno_id}_results.png'),
                 os.path.join(mesh_norm_dir, f'{anno_id}_our_results.png'))


if args.vis_wavelet_gen:
    from utils import visualize
    results_dir = os.path.join(results_dir, 'wavelet', 'generation')
    misc.check_dir(results_dir)

    mesh_dir = '/projects/hsa/results/occflexi/occflexi_25/2024-09-03_12-48-01_Network-Marching-Cubes-Diffusion-Gen'
    mesh_names = list(filter(lambda x: os.path.splitext(x)[1] == '.obj',
                             os.listdir(mesh_dir)))
    print(mesh_names)
    for mn in mesh_names:
