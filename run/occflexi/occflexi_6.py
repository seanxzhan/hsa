...
def sigmoid_approximation(x, k=20):
    return 1 / (1 + torch.exp(-k * (x - 0.5)))

def tanh_approximation(x, k=50):
    return 0.5 * (torch.tanh(k * (x - 0.5)) + 1)
...
if args.train:
    for it in range(iterations):
        ...
        comp_sdf = -pred_verts_occ.view(-1, fc_res+1, fc_res+1, fc_res+1)
        ...
        total_loss = occ_loss + mesh_loss + torch.norm(pred_occ)
