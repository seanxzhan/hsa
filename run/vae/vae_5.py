...
from occ_networks.encoder_decoder_nasa_5 import VAE
...
partnet_dataset = PartPtsDataset(
    root=f'data/{cat_name}_train_{pt_sample_res}_11_{ds_start}_{ds_end}_whole').to(device)
if OVERFIT:
...
def loss_f(pred_values, gt_values, mu, log_var, beta):
    recon_loss = mse(pred_values, gt_values)
    kl_loss = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp())
    return recon_loss + 0.001 * kl_loss


def train_one_itr(partnet_data, batch_points, batch_values, beta):
    occs, mu, log_var = model.forward(partnet_data.pos, partnet_data.batch,
                                      batch_points)
    loss = loss_f(occs, batch_values, mu, log_var, beta)
